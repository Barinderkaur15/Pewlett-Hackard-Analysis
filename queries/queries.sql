...
----to find retirement eligible employeed who are actually working 
SELECT ri.emp_no,
	ri.first_name,
	ri.last_name,
     de.to_date
...
-- to check name of distinct department 
select distinct(Dept_name) from dept_info;


select * from retirement_info
-- Employees from sales department only 
...
select dept_name, count(*) from dept_info where dept_name in('Sales','Development') group by dept_name
